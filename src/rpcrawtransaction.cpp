...
Value getrawtransaction(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "           \"powocoinaddress\"        (string) powocoin address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
...
Value listunspent(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "3. \"addresses\"    (string) A json array of powocoin addresses to filter\n"
            "    [\n"
            "      \"address\"   (string) powocoin address\n"
            "      ,...\n"
            "    ]\n"
            "\nResult\n"
            ...
            "    \"address\" : \"address\",  (string) the powocoin address\n"
    ...
    if (params.size() > 2) {
        ...
        BOOST_FOREACH (Value& input, inputs) {
            ...
            if (!address.IsValid())
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Powocoin address: ") + input.get_str());
...
Value createrawtransaction(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\": x.xxx   (numeric, required) The key is the powocoin address, the value is the btc amount\n"
            "      ,...\n"
            "    }\n"
...
Value decoderawtransaction(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "         \"type\" : \"pubkeyhash\",  (string) The type, eg 'pubkeyhash'\n"
            "         \"addresses\" : [           (json array of string)\n"
            "           \"12tvKAXCxZjSmdNbao16dKXC8tRWfcF5oc\"   (string) powocoin address\n"
            "           ,...\n"
            "         ]\n"
            "       }\n"
            "     }\n"
