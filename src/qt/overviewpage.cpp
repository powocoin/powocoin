...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::POWO)
    {
    }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("POWO")
    updateDisplayUnit();
}
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizePowocoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizePowocoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizePowocoinAmount = strAnonymizePowocoinAmount.remove(strAnonymizePowocoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizePowocoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizePowocoinAmount * COIN) nMaxToAnonymize = nAnonymizePowocoinAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizePowocoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizePowocoinAmount));
        strAnonymizePowocoinAmount = strAnonymizePowocoinAmount.remove(strAnonymizePowocoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizePowocoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
