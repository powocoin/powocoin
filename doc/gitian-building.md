...
*Setup instructions for a gitian build of Powocoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Powocoin
...
- [Building Powocoin](#building-powocoin)
...
This POWO image can be validated using a SHA256 hashing tool, for example on
...
Then start the VM. On the first launch you will be asked for a CD or POWO image. Choose the downloaded iso.
...
Clone the git repositories for powocoin and gitian and then checkout the powocoin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/powocoin-crypto/powocoin.git
cd powocoin
...
Follow the instructions in [doc/release-process.md](release-process.md) in the powocoin repository
...
To build Powocoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the powocoin repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/powocoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/powocoin-crypto/powocoin
...
For example:
```bash
URL=https://github.com/crowning-/powocoin.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit powocoin=${COMMIT} --url powocoin=${URL} ../Powocoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit powocoin=${COMMIT} --url powocoin=${URL} ../Powocoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit powocoin=${COMMIT} --url powocoin=${URL} ../Powocoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/powocoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/powocoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/powocoin-build.assert
```
...
[powocoin/gitian.sigs](https://github.com/powocoin-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
